...
type Item = { codigo: string; nombre: string; perVal: number; ytdVal: number; perPct: number; ytdPct: number };

type ErData = {
  ingresos: Item[]; costos: Item[];
  ...
  ventasPer: number; ventasYTD: number;
  totalIngresosPer: number; totalIngresosYTD: number;
  totalCostosPer: number; totalCostosYTD: number;
  totalGastosPer: number; totalGastosYTD: number;
  totalOtrosIngresosPer: number; totalOtrosIngresosYTD: number;
  totalOtrosGastosPer: number; totalOtrosGastosYTD: number;
  utilidadBrutaPer: number; utilidadBrutaYTD: number;
  utilidadOperacionPer: number; utilidadOperacionYTD: number;
...
export function generateResultadosPDF(org: { razon_social: string; rfc: string; regimen?: string }, er: ErData, desde: number, hasta: number, ejercicio: number, detalle = true) {
  ...
  const col1 = colW * 0.40;
  const col2 = colW * 0.15;
  const col3 = colW * 0.10;
  const col4 = colW * 0.20;
  ...
  function totalRow(label: string, perVal: number, ytdVal: number, opts?: { color?: [number, number, number]; topBorder?: boolean; big?: boolean; percentage?: boolean }) {
    ...
    autoTable(doc, {
      startY: (doc as any).lastAutoTable?.finalY ?? 118,
      body: [[
        { content: label, styles: { fontStyle: "bold", fontSize: opts?.big ? 10 : 8.5, cellPadding: opts?.big ? [6, 4, 6, 4] : [4, 4, 4, 4] } },
        { content: fm(perVal), styles: { halign: "right", fontStyle: "bold", fontSize: opts?.big ? 10 : 8.5, textColor: c, cellPadding: opts?.big ? [6, 4, 6, 4] : [4, 4, 4, 4] } },
        { content: opts?.percentage !== false ? `${pctVal.toFixed(2)}%` : "", styles: { halign: "right", fontSize: opts?.big ? 9 : 7.5, textColor: 100, cellPadding: opts?.big ? [6, 4, 6, 4] : [4, 4, 4, 4] } },
        { content: fm(ytdVal), styles: { halign: "right", fontStyle: "bold", fontSize: opts?.big ? 10 : 8.5, textColor: c, cellPadding: opts?.big ? [6, 4, 6, 4] : [4, 4, 4, 4] } },
        { content: opts?.percentage !== false ? `${ytdPctVal.toFixed(2)}%` : "", styles: { halign: "right", fontSize: opts?.big ? 9 : 7.5, textColor: 100, cellPadding: opts?.big ? [6, 4, 6, 4] : [4, 4, 4, 4] } },
      ]],
      margin: { left: margin, right: margin },
      tableLineWidth: 0,
  ...
  function spacer(h = 6) {
    ...
    autoTable(doc, {
      ...
      columnStyles: { 0: { cellWidth: col1 }, 1: { cellWidth: col2 }, 2: { cellWidth: col3 }, 3: { cellWidth: col4 }, 4: { cellWidth: col5 } },
    });
  }
  ...
  totalRow("Total Otros Ingresos", er.totalOtrosIngresosPer, er.totalOtrosIngresosYTD, { color: [5, 150, 105] });
  totalRow("Total Otros Gastos", -er.totalOtrosGastosPer, -er.totalOtrosGastosYTD, { color: [147, 51, 234] });
