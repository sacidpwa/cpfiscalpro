...
export const getHelixLarossSplit = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const asimiladosAccts = ["610000200000000000002"]; // Asimilados a Salarios (honorarios profesionales)
    const imssAccts = ["610001700000000000002", "620001600000000000002"]; // Cuota IMSS (gasto patronal)
    ...
    const antAsimilados =
      data.desdeMes > 1 ? await getBalSum(asimiladosAccts, data.desdeMes - 1) : 0;
    ...
    return {
      helix: {
        ...
        asimilados: asimiladosPer * hRatio,
        imss: imssPer * hRatio,
        isn: isnPer * hRatio,
      ...
      laross: {
        ...
        asimilados: asimiladosPer * lRatio,
        imss: imssPer * lRatio,
        isn: isnPer * lRatio,
