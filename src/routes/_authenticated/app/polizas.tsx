...
const TIPO_STYLES: Record<string, string> = {
  ...
  transferencia: "bg-violet-100 text-violet-700 dark:bg-violet-900/40 dark:text-violet-300",
};
...
function Polizas() {
  ...
  const getEntry = useServerFn(getJournalEntry);
  ...
  const [desdeCustom, setDesdeCustom] = useState(() =>
    new Date(anioActual, mesActual - 1, 1).toISOString().slice(0, 10),
  );
  const [hastaCustom, setHastaCustom] = useState(() =>
    new Date(anioActual, mesActual, 0).toISOString().slice(0, 10),
  );
  ...
  const { data: accounts } = useQuery({
    ...
    queryFn: () => accountsFn({ data: { organizationId: org.id } }),
  });
  const [editEntry, setEditEntry] = useState<any>(null);
  const open = editEntry !== null;
  ...
  async function save(p: any) {
    try {
      await upsert({ data: { ...p, organizationId: org.id, id: editEntry?.id ?? undefined } });
      toast.success(editEntry ? "Póliza actualizada" : "Póliza registrada");
  ...
  return (
    <div>
      <PageHeader
        title="Pólizas"
        description="Captura con validación de cuadre automático"
        actions={
          <button
      ...
      <div className="space-y-4 p-8">
        <div className="flex flex-wrap items-center gap-3">
          ...
          {["", "ingreso", "egreso", "diario", "cheque", "transferencia"].map((t) => (
            <button
              key={t}
          ...
          <div className="flex items-center gap-1.5 border-l pl-3">
            ...
            <button
              onClick={() => setFiltroFecha(filtroFecha === "mes" ? "personalizado" : "mes")}
              className="rounded-md border px-2 py-1 text-xs font-medium bg-card hover:bg-secondary/50"
            >
              {filtroFecha === "mes" ? "Mes" : "Personalizado"}
            </button>
            {filtroFecha === "mes" ? (
              <>
                <select
            ...
            <table className="w-full text-sm">
              <thead className="bg-secondary/50 text-xs uppercase text-muted-foreground">
                <tr>
                  ...
                  <th className="px-3 py-2 text-left">Núm.</th>
                ...
                {data.map((p: any) => (
                  ...
                  >
                    ...
                    <td className="px-3 py-2">
                      <span
                        className={`rounded px-1.5 py-0.5 text-xs font-medium ${TIPO_STYLES[p.tipo] || "bg-secondary text-muted-foreground"}`}
                      >
                    ...
                    <td className="px-3 py-2">
                      <span
                        className={`rounded px-1.5 py-0.5 text-xs capitalize ${p.estatus === "cancelada" ? "bg-destructive/10 text-destructive" : "bg-success/10 text-success"}`}
                      >
                        {p.estatus}
                      </span>
                    </td>
...
function PolizaForm({ entry, accounts, onClose, onSave }: any) {
  const editing = entry?.id;
  const [tipo, setTipo] = useState(entry?.tipo ?? "diario");
  const [fecha, setFecha] = useState(
    entry?.fecha?.slice(0, 10) ?? new Date().toISOString().slice(0, 10),
  );
  const [concepto, setConcepto] = useState(entry?.concepto ?? "");
  const [referencia, setReferencia] = useState(entry?.referencia ?? "");
  const [lines, setLines] = useState(() => {
    ...
    return [
      { account_id: "", concepto: "", cargo: 0, abono: 0 },
      { account_id: "", concepto: "", cargo: 0, abono: 0 },
    ];
  });
  ...
  function submit(e: React.FormEvent) {
    e.preventDefault();
    if (!cuadra) {
      toast.error("La póliza no cuadra");
      return;
    }
    const valid = lines.filter((l) => l.account_id && (Number(l.cargo) > 0 || Number(l.abono) > 0));
    if (valid.length < 2) {
      toast.error("Necesitas al menos 2 movimientos");
      return;
    }
    onSave({
      tipo,
      ...
      concepto,
      referencia,
      lines: valid.map((l) => ({ ...l, cargo: Number(l.cargo), abono: Number(l.abono) })),
    });
  }
  ...
  return (
    <div className="fixed inset-0 z-50 grid place-items-center bg-black/40 p-4" onClick={onClose}>
      <form
        ...
        className="max-h-[90vh] w-full max-w-5xl overflow-auto rounded-lg border bg-card p-6"
      >
        <div className="mb-4 flex items-center justify-between">
          <h2 className="text-lg font-semibold">{editing ? "Editar póliza" : "Nueva póliza"}</h2>
        ...
        <div className="grid grid-cols-4 gap-3">
          ...
          <Lbl label="Concepto" className="col-span-2">
            <input
              className="inp"
              value={concepto}
          ...
          <Lbl label="Referencia" className="col-span-4">
            <input
              className="inp"
              value={referencia}
        ...
        <div className="overflow-hidden rounded-md border">
          <table className="w-full text-sm">
            <thead className="bg-secondary/40 text-xs text-muted-foreground">
              <tr>
                <th className="px-2 py-1.5 text-left" style={{ minWidth: 340 }}>
                  Cuenta
                </th>
                <th className="px-2 py-1.5 text-left">Concepto</th>
                <th className="px-2 py-1.5 text-right">Cargo</th>
                <th className="px-2 py-1.5 text-right">Abono</th>
                <th></th>
              </tr>
            </thead>
            <tbody className="divide-y">
              {lines.map((l, i) => (
                <tr key={i}>
                  <td className="p-1" style={{ minWidth: 340 }}>
                    <select
                      className="inp"
                      value={l.account_id}
                  ...
                  <td className="p-1">
                    <input
                      ...
                      className="inp text-right font-mono"
                      value={l.abono || ""}
                      onChange={(e) => update(i, "abono", e.target.value)}
                    />
                  </td>
                  <td className="p-1">
                    <button
            ...
            <tfoot className="bg-secondary/30 text-sm font-semibold">
              ...
              <tr className={cuadra ? "text-success" : "text-destructive"}>
                ...
                <td colSpan={2} className="px-2 py-1.5 text-right text-money">
        ...
        <button
          type="button"
          onClick={() => setLines([...lines, { account_id: "", concepto: "", cargo: 0, abono: 0 }])}
          className="mt-2 rounded-md border bg-card px-2 py-1 text-xs hover:bg-secondary"
        >
          + Agregar movimiento
        </button>
