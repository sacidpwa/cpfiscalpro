        ...
        <div className="max-w-sm rounded-lg border bg-card p-6 text-center shadow-sm">
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            {(error as Error | undefined)?.message ??
              "La sesión está activa, pero la carga de permisos falló."}
          </p>
          <button
            onClick={() => refetch()}
...
function RouterAware() {
  ...
  const onAllowedNoOrgRoute = path === "/onboarding" || path.startsWith("/admin");
      ...
      return (
        <div className="grid min-h-screen place-items-center p-6 text-center">
          <div className="max-w-sm rounded-lg border bg-card p-6 shadow-sm">
            <h1 className="text-lg font-semibold text-foreground">
              No se pudo validar el panel admin
            </h1>
            <p className="mt-2 text-sm text-muted-foreground">
              Actualiza la página e intenta entrar de nuevo.
            </p>
            <Link
              to="/admin"
...
const NAV: NavItem[] = [
  ...
  { to: "/app/flujo-efectivo", label: "Flujo Efectivo", icon: Waves, module: "contabilidad" },
  ...
  {
    to: "/app/facturacion/complementos",
    label: "Complementos",
    icon: Truck,
    module: "facturacion",
  },
...
function Shell() {
  ...
  const currentLabel =
    navVisible.find((n) => (n.exact ? path === n.to : path === n.to || path.startsWith(n.to + "/")))
      ?.label ?? "Dranur";
    ...
    <div className="min-h-screen bg-background">
      ...
      >
        <div className="flex h-14 items-center justify-between gap-2 border-b border-sidebar-border px-4">
          ...
          >
            ...
            <span
              className={`font-semibold tracking-tight overflow-hidden whitespace-nowrap transition-all duration-500 ease-[cubic-bezier(0.4,0,0.2,1)] ${sidebarCollapsed ? "max-w-0 opacity-0" : "max-w-[200px] opacity-100"}`}
            >
              CPFiscalPro
            </span>
          </div>
          ...
          {navVisible.map((n) => {
            ...
            return (
              ...
              >
                ...
                <span
                  className={`overflow-hidden whitespace-nowrap transition-all duration-500 ease-[cubic-bezier(0.4,0,0.2,1)] ${sidebarCollapsed ? "max-w-0 opacity-0" : "max-w-[200px] opacity-100"}`}
                >
                  {n.label}
                </span>
              </Link>
      ...
      <main className="min-w-0 lg:ml-14">
        {billing && billing.adeudoTotal > 0 && (
          <div
            className={`flex items-center gap-2 border-b px-4 py-2 text-xs sm:px-6 ${adeudoGrave ? "bg-destructive/10 text-destructive" : "bg-amber-500/10 text-amber-700 dark:text-amber-300"}`}
          >
            ...
            <Link to="/app/cobranza" className="ml-auto underline">
              Ver detalle
            </Link>
          </div>
        )}
...
function UserMenu({ collapsed }: { collapsed?: boolean }) {
  ...
  return (
    ...
    >
      ...
      <span
        className={`overflow-hidden whitespace-nowrap transition-all duration-500 ease-[cubic-bezier(0.4,0,0.2,1)] ${collapsed ? "max-w-0 opacity-0" : "max-w-[200px] opacity-100"}`}
      >
        Cerrar sesión
      </span>
    </button>
